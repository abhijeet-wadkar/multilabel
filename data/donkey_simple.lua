...
function dataset:__init(args)
  ...
  inputFilePtr = io.open(args.data_list, 'r')
  text = inputFilePtr:read()
  inputFilePtr:close()
  allImages = cjson.decode(text)
  
  for idx = 1, #allImages do
    self.data:insert(allImages[idx].path)
    self.label:insert(tds.Vec(allImages[idx].label))
  end
  --[[
...
function dataset:tableToOutput(dataTable, scalarTable, extraTable)
   ...
   scalarLabels = torch.LongTensor(quantity, self.nClasses):fill(0)
   for i=1,#dataTable do
      data[i]:copy(dataTable[i])
      
      for j=1, #scalarTable[i] do
        scalarLabels[i][scalarTable[i][j]] = 1
      end
      -- scalarLabels[i] = scalarTable[i]
   end
   -- print(scalarLabels[1])
